...
Taiga 敏捷项目管理系统是一个基于 Python/Django 语言、采用前后端分离架构（B/S 模式）、应用 Django REST Framework 开发框架和 PostgreSQL 数据库技术实现的 Web 应用系统。本文档基于 Taiga 开源项目（版本 6.9.0）的源代码进行深入分析和文档化工作，旨在为学习软件工程的读者提供一个完整的、可参考的实践案例。
...
Taiga 敏捷项目管理系统是一个基于 Python 和 TypeScript/CoffeeScript 语言、在前后端分离的 B/S 架构模式下运行、并采用 Django REST Framework 开发框架和 PostgreSQL 数据库技术实现的 Web 应用系统。

该系统将传统的软件项目管理模式升级为现代化的敏捷项目管理模式，其功能主要有两个方面。一方面通过 Scrum 敏捷框架，实现产品待办列表管理、冲刺规划、任务分解与燃尽图追踪等功能，支持团队进行迭代式开发；另一方面通过 Kanban 看板方法，实现可视化的工作流管理、WIP（在制品）限制和持续交付流程优化。

从项目管理者的角度看，使用该系统能够使管理人员更方便地查看项目进度情况、团队成员的工作负载以及迭代完成情况等基本信息。基于系统记录的燃尽数据和速度统计，管理者能够制定更合理的迭代计划和资源分配策略，对工作量的把握更为精准，有效减少了因计划失误造成的项目延期风险，大大提高了项目的交付成功率。从开发团队成员的角度看，使用该系统能方便地查看自己负责的任务、了解任务的优先级和依赖关系、及时更新任务状态，从而使团队协作更为高效，减少了沟通成本和信息不对称。此举方便了管理者和开发者，提高了团队的敏捷化水平，且有利于组织持续改进开发流程，间接提高了软件产品质量。
...
用户认证流程基于 JWT (JSON Web Token) 机制实现，核心代码位于 `taiga/auth/api.py`。
...
1. **注册流程** (`/api/v1/auth/register`):
   ...
   - 调用 `public_register()` 或 `private_register_for_new_user()` 创建用户
...
2. **登录流程** (`/api/v1/auth`):
   ...
   - 通过 `TokenObtainPairSerializer` 验证并生成 JWT Token
...
项目创建通过 `ProjectViewSet` 实现，支持从预设模板初始化项目配置。
...
Sprint 管理核心代码位于 `taiga/projects/milestones/services.py`。

**流程说明**：

1. **创建 Sprint**：设置名称、起止日期
2. **规划用户故事**：从 Backlog 拖拽用户故事到 Sprint
   - 调用 `update_userstories_milestone_in_bulk()` 批量更新
   - 同步更新关联任务的 `milestone` 字段
3. **任务分解**：为每个用户故事创建子任务
4. **执行 Sprint**：每日更新任务状态
5. **燃尽图计算**：根据剩余故事点动态计算进度曲线
6. **Sprint 完成判断**：
   - 调用 `calculate_milestone_is_closed()` 检查所有故事/任务/问题是否完成
   - 若完成则调用 `close_milestone()` 关闭 Sprint

![Sprint规划与燃尽图流程](drawio/ch1/1_3_sprint_flow.png)

##### (4) Kanban 模式：任务卡片流转与泳道管理流程

Kanban 模式通过泳道 (Swimlane) 和状态列实现可视化管理，核心逻辑在 `userstories/services.py`。
...
4. 卡片拖拽流转：
   - 调用 `update_userstories_kanban_order_in_bulk()` 更新状态和泳道
   - 支持 `before_userstory` / `after_userstory` 定位
5. 状态变更时触发 `recalculate_is_closed_for_userstory_and_its_milestone()`

![Kanban任务卡片流转流程](drawio/ch1/1_4_kanban_flow.png)

##### (5) 用户故事 (User Story) 全生命周期管理流程

用户故事是 Taiga 的核心业务对象，完整生命周期管理代码位于 `userstories/services.py`。

**生命周期状态**：

| 状态           | 说明             | 关键函数                                               |
| -------------- | ---------------- | ------------------------------------------------------ |
| New            | 新创建           | `create_userstories_in_bulk()`                         |
| Backlog        | 在待办列表中排序 | `update_userstories_order_in_bulk()`                   |
| Sprint         | 已规划到冲刺     | `update_userstories_backlog_or_sprint_order_in_bulk()` |
| In Progress    | 开发中           | 状态变更 API                                           |
| Ready for Test | 待测试           | 状态变更 API                                           |
| Done           | 已完成           | `close_userstory()`                                    |
| Blocked        | 阻塞中           | `is_blocked=True`                                      |

![用户故事生命周期流程](drawio/ch1/1_5_userstory_lifecycle.png)

##### (6) 问题 (Issue) 追踪与处理流程

问题追踪模块位于 `taiga/projects/issues/`，用于 Bug 报告和支持请求管理。
...
史诗 (Epic) 用于管理跨多个 Sprint 的大型功能，代码位于 `taiga/projects/epics/`。

**流程说明**：

1. 创建 Epic：设置标题、描述、颜色标识
2. 关联用户故事：通过 `EpicRelatedUserStoryViewSet` 建立关联
...
数据流图采用分层的方式进行绘制，从顶层（Context Diagram）开始，逐层细化到更详细的处理过程。本节基于 Taiga 源码中的模块划分和 API 路由设计（`routers.py`）绘制三层数据流图，如图 1.8 至图 1.10 所示。
...
| 处理过程编号 | 名称     | 对应模块                                               |
| ------------ | -------- | ------------------------------------------------------ |
| 1.0          | 用户认证 | `taiga/auth/`, `taiga/users/`                          |
| 2.0          | 项目管理 | `taiga/projects/api.py`                                |
| 3.0          | 敏捷管理 | `taiga/projects/userstories/`, `milestones/`, `tasks/` |
| 4.0          | 问题追踪 | `taiga/projects/issues/`                               |
| 5.0          | 知识库   | `taiga/projects/wiki/`                                 |
...
经过对 Taiga 源码的分析，该项目需要建立用户表、项目表、用户故事表、任务表、冲刺表、问题表、史诗表、成员角色表、Wiki 页面表等，其初步的数据字典如下表 1.4 至表 1.12 所示：

##### 表 1.4 用户 (User) 数据字典

**数据表名称**: `users_user`  
**源文件**: `taiga/users/models.py`
...
**数据表名称**: `projects_project`  
**源文件**: `taiga/projects/models.py`
...
**数据表名称**: `userstories_userstory`  
**源文件**: `taiga/projects/userstories/models.py`
...
**数据表名称**: `tasks_task`  
**源文件**: `taiga/projects/tasks/models.py`
...
**数据表名称**: `milestones_milestone`  
**源文件**: `taiga/projects/milestones/models.py`
...
**数据表名称**: `issues_issue`  
**源文件**: `taiga/projects/issues/models.py`
...
**数据表名称**: `epics_epic`  
**源文件**: `taiga/projects/epics/models.py`
...
**数据表名称**: `wiki_wikipage`  
**源文件**: `taiga/projects/wiki/models.py`
...
本节基于 Taiga 源码中的 Django Model 定义，使用实体-关系图 (Entity-Relationship Diagram) 描述系统核心数据结构。全局的 E-R 图设计如图 2.1 所示，对全局 E-R 图进行分解，得到图 2.2 至图 2.5 所示的局部 E-R 图。
...
如下图 2.6 顶层数据流图所示，该系统整体上可以分为三个实体和一个处理过程。其实体分别为普通用户、项目管理员、系统管理员，处理过程为 Taiga 敏捷项目管理系统。该数据流图以人机交互思想为指导，从整体观念出发，展示了流入和流出系统的所有数据信息。
...
**数据表名称**: `notifications_notifypolicy`  
**用途**: 用户针对每个项目的通知偏好设置
...
本章将对 Taiga 敏捷项目管理系统进行总体设计，主要包括系统结构图、数据库设计和模块 IPO 表三个部分。系统结构图展示了系统的整体架构和模块划分；数据库设计包括逻辑结构设计和物理结构设计；模块 IPO 表则详细描述了各主要模块的输入、处理和输出。
...
后端采用 Django 应用模块化设计，核心模块在 `taiga/` 目录下。
...
| **关键函数** | `public_register()`, `get_token()` |
...
| **输出**     | 更新后的用户故事列表                                            |
| **关键函数** | `update_userstories_order_in_bulk()`                            |
...
| **输出**     | Milestone 及关联故事                                                   |
| **关键函数** | `update_userstories_milestone_in_bulk()`                               |
...
| **输出**     | 更新后的用户故事                                                                      |
| **关键函数** | `update_userstories_kanban_order_in_bulk()`                                           |
...
| **输出**     | Membership 对象                                                    |
| **关键函数** | `add_member()`, `accept_invitation()`                              |
...
```python
# taiga/projects/userstories/models.py
...
本文档对 Taiga 敏捷项目管理系统进行了全面的软件工程分析，主要内容包括：
