...
| 前端框架   | AngularJS 1.5 + CoffeeScript | Angular 21                |
...
Taiga 系统采用现代化的前后端分离架构，技术选型如表 1.1 所示：

**表 1.1 Taiga 系统技术架构表**

| 层次        | 技术组件              | 版本       | 说明               |
| ----------- | --------------------- | ---------- | ------------------ |
| 前端框架    | AngularJS             | 1.5.10     | 单页应用框架       |
| 前端语言    | CoffeeScript          | 1.10.0     | 编译为 JavaScript  |
| 前端构建    | Gulp                  | 4.0.2      | 自动化构建工具     |
| 前端样式    | SASS                  | 8.0.0      | CSS 预处理器       |
| 后端框架    | Django                | 3.2.25 LTS | Python Web 框架    |
| API 框架    | Django REST Framework | 3.14+      | RESTful API 构建   |
...
为了更清晰地了解 Taiga 系统的定位和优势，表 1.2 列出了与市场上主流敏捷项目管理工具的对比：
...
基于上述分析的七个系统模块，本项目系统的业务流程图如下图 1.1 至图 1.7 所示。每张业务流程图均基于 Taiga 源代码中的 API 实现和服务层逻辑进行绘制。
...
- **外部系统**：GitHub、GitLab、Jira 等第三方平台，通过 Webhook 与 Taiga 交互
...
| total_milestones     | INTEGER      | NULL            | 里程碑总数   |
...
本项目采用的开发工具为 PostgreSQL、Visual Studio Code 和 PyCharm，实现基于 Django REST Framework 的 Python 后端完全能够胜任此次开发。因为本系统服务的用户阈值为中小型软件开发团队（通常 5-50 人），考虑到用户同时操作项目的并发性不是非常高，而 PostgreSQL 是一个功能强大的开源关系型数据库管理系统，被广泛应用于各类 Web 应用中。由于其对 JSONB 的原生支持、强大的全文检索能力、以及完善的事务处理机制，本项目选择了 PostgreSQL 作为后端数据库。

Django 是一个高级 Python Web 框架，其"开箱即用"的设计理念使得开发者能够快速构建安全、可维护的 Web 应用。Django REST Framework 在 Django 基础上提供了强大的 API 开发能力，使本系统能够以标准化的 RESTful 风格提供接口服务。
...
基于 `requirements.txt` 分析，Taiga 后端采用成熟稳定的技术栈：
...
| 技术         | 版本   | 用途              |
| ------------ | ------ | ----------------- |
| AngularJS    | 1.5.10 | MVC 框架          |
| CoffeeScript | 1.10.0 | JavaScript 预处理 |
| Gulp         | 4.0.2  | 构建工具          |
| SASS         | 8.0.0  | CSS 预处理器      |
...
- AngularJS 双向数据绑定简化开发
- CoffeeScript 提高代码可读性
- Gulp 自动化构建流程
...
全局 E-R 图展示 Taiga 系统中所有核心实体及其关联关系。
...
本节给出 Taiga 系统的总体结构图，包括前后端分离架构图、后端模块依赖图和前端模块依赖图。
...
Taiga 采用前后端分离的 SPA (Single Page Application) 架构。

**架构层次**：

| 层次   | 技术栈                   | 职责               |
| ------ | ------------------------ | ------------------ |
| 前端层 | AngularJS + CoffeeScript | 用户界面、交互逻辑 |
| API 层 | Django REST Framework    | RESTful API 服务   |
...
前端采用 AngularJS 模块化组织，模块在 `app/modules/` 目录下。
...
本节对 Taiga 系统的数据库进行详细设计，包括逻辑结构设计和物理结构设计两个部分。逻辑结构设计主要描述数据库表结构和表之间的关系；物理结构设计主要描述索引策略、存储优化和性能配置。
...
IPO 表（输入-处理-输出表）是描述系统各模块功能的有效工具。通过 IPO 表可以清晰地了解每个模块的输入数据、处理逻辑和输出结果。本节给出 Taiga 系统主要模块的 IPO 表，包括用户认证、项目管理、敏捷管理等核心模块。
...
本章将对 Taiga 系统各功能模块进行详细设计。详细设计是在总体设计的基础上，对每个模块的内部处理逻辑进行具体描述，包括模块的输入输出、处理流程、算法设计和接口定义等。本章共分为 9 个小节，分别对应系统的主要功能模块。
...
本章将详细介绍 Taiga 系统的实现过程，包括开发环境配置、核心编码实现、各模块功能实现以及关键算法实现四个部分。通过具体的代码示例展示系统的实现细节。

### 5.1 开发平台和开发环境介绍

本节介绍 Taiga 系统的开发平台和开发环境配置，包括后端 Python/Django 环境和前端 Node.js/AngularJS 环境。
...
- Taiga 开源版：taiga.etao.net
- Tenzu 新版：se.etao.net（登录：1user/123123）
...
本章将介绍 Taiga 系统的测试策略和测试过程，包括测试目标、测试步骤、测试策略以及具体的测试用例。测试是软件开发过程中的重要环节，通过系统性的测试可以发现和修复缺陷，确保软件质量。
...
本章将介绍 Taiga 系统的维护过程和维护策略。软件维护是软件生命周期中持续时间最长、成本最高的阶段，良好的维护策略可以延长软件的使用寿命，保障系统的稳定运行。
...
1. **可行性研究**: 完成技术、经济可行性分析，证明 Taiga 技术栈成熟可靠
...
- Django REST Framework 构建标准化 API
...
通过对 Taiga 系统的深入分析，体会到：

1. **开源软件的价值**: Taiga 作为开源项目，代码质量高，架构清晰，是学习软件工程的优秀案例
2. **敏捷方法论的实践**: Taiga 不仅是敏捷工具，其开发过程本身也遵循敏捷原则
